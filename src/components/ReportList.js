...
const ReportList = () => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        const response = await axios.get('https://ecs-backend-xsuv.onrender.com/api/reports');
  ...
  const handleDelete = async (reportId) => {
    try {
      await axios.delete(`https://ecs-backend-xsuv.onrender.com/api/reports/${reportId}`);
